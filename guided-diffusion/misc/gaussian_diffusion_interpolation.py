...
class GaussianDiffusion:
    ...
    def p_sample_loop_progressive(
        ...
        for int_i in range(1000):
            ...
            if(int_i%125==0 or int_i==999):
                ...
                for step in range(11):
                    ...
                    """sample = ((save_noise_img + 1) * 127.5).clamp(0, 255).to(th.uint8)
                    sample = sample.permute(0, 2, 3, 1)
                    sample = sample.contiguous()
                    sample = sample.cpu().numpy()
                    img = Image.fromarray(sample[0], 'RGB')
                    plt.imshow(img, interpolation='none')
                    #plt.show()
                    
                    sample = ((save_noise_img2 + 1) * 127.5).clamp(0, 255).to(th.uint8)
                    sample = sample.permute(0, 2, 3, 1)
                    sample = sample.contiguous()
                    sample = sample.cpu().numpy()
                    img = Image.fromarray(sample[0], 'RGB')
                    ...
                    #print(img)
                    out_img.save(f'generated_imgs/{int_i}-interpolate-{step}.jpg') # change to .eps for PDF
                    out_img.save(f'generated_imgs/{int_i}-interpolate-{step}.eps') # change to .eps for PDF
                    #img.save(f'generated_imgs/{int_i}-interpolate-{step}.svg') # change to .eps for PDF
        ...
        img1 = th.load(f'source-0-noised{self.num_timesteps}.pt', map_location=th.device(device))
        img2 = th.load(f'source-1-noised{self.num_timesteps}.pt', map_location=th.device(device))
        ...
        for int_i in range(interpolation_steps):
            ...
            img = (slerp_theta(img1,img2,int_i*0.1*math.pi/2))
            ...
            for i in indices:
                ...
                if(i==9999):
                    th.save(img,f"generated_imgs/th_files/{i}-source-{int_i}.pt")
                ...
                with th.no_grad():
                    ...
                    if(make_intermediate_and_gif):
                        if(i%125==10000):
                            th.save(out["sample"],f"generated_imgs/th_files/{i}-source-{int_i}.pt")
                        ...
                        # assume batch size of 1,
                            ...
                            img = Image.fromarray(sample[0], 'RGB')
                            img.save(f'generated_imgs/{self.num_timesteps}-interpolate-{int_i}.jpg') # change to .eps for PDF
                            img.save(f'generated_imgs/{self.num_timesteps}-interpolate-{int_i}.eps') # change to .eps for PDF
                            #img.save(f'generated_imgs/source-{int_i}_img_size_128x128.jpg') # change to .eps for PDF
                            #img.save(f'generated_imgs/source-{int_i}_img_size_128x128.eps') # change to .eps for PDF
    ...
    def ddim_sample_loop_progressive(
        ...
        #img1 = th.load(f'generated_imgs/th_files/{self.num_timesteps}-source-0.pt', map_location=th.device(device))
        #img2 = th.load(f'generated_imgs/th_files/{self.num_timesteps}-source-1.pt', map_location=th.device(device))

        print("We are using ddim_sample_loop_progressive")
        images = []

        #for i in range(11):
            #images.append(slerp_theta(img1,img2,i*0.1*math.pi/2))

        for int_i in range(11):
...
def interpolate(self, x1, x2, t = None, lam = 0.5):
        ...
        t_batched = th.stack([th.tensor(t, device = device)] * b)
        print(t_batched)
